...
MODEL = "model_with_extended_dataset_resnet50_2023-03-27_16-08-23.pth"
SESSION_ID = None

classifier = classifier = MLADHD()
classifier.load_model(os.path.join(CLASSIFIER_PATH, MODEL))
...
def take_screenshot():
    ...
    pyautogui.screenshot(os.path.join(SCREENSHOT_PATH, filename), format='jpg')
    SCREENSHOT_COUNTER += 1
    return filename

def process_screenshot(filename):
...
def log(log_dir, session_id, text, pred, prob):
...
def main():
    ...
    while True:
        ...
        img = process_screenshot(screenshot_filename)
        contours = bounding_box(img)
        text = OCR(img, contours, include_annotations=False)
        print("Text extracted")
        print("Detecting distraction")
        pred, prob = distraction_detection(img)
        if pred == 1:
            print("Predicted: distracted")
        else:
            print("Predicted: focused")
        
        log(LOG_PATH, SESSION_ID, screenshot_filename, text, pred, prob)
